...
def save(filename):
    idx = len(pd.read_csv("database_forText.csv"))
    new_df = pd.DataFrame({"filename": filename},
                          index=[idx])
    new_df.to_csv("database_forText.csv", mode="a", header=False)
    return None


def load_list():
    house_list = []
    df = pd.read_csv("database_forText.csv")
...
def delete_all_files(filePath):     # 폴더 내 전체 파일 삭제 (ㅠㅠ csv 내용 초기화하는거는 포기,,)
    ...
    # iris = pd.read_csv("database_forText.csv") 
    # if (now_index()>0):
    #     <방법1>
